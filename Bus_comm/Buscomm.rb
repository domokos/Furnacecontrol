  ...
  @busmutex.synchronize do
    ...
    while true
     ...
     #Create the message
      ...
      retry_count += 1
      $app_logger.warn("Messaging retry #"+retry_count.to_s+" Error code: "+@response[:Return_code].to_s+" - "+RESPONSE_TEXT[@response[:Return_code]]+" Device return code: "+retval[:DeviceResponseCode].to_s) if retry_count > 1
